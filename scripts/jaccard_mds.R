...
library(ggrepel)
ggplot(data = fit_and_meta_df, aes(x = V1, y = V2, label = AbbreviatedName, shape = TopLevel, color = Type)) +
  geom_point(size=2) + 
  geom_text_repel(
    size=3,
...
ggplot(data = fit_and_meta_df, aes(x = V2, y = V3, label = AbbreviatedName, shape = TopLevel, color = Type)) +
  geom_point(size=2) + 
  geom_text_repel(
    size=3,
    show.legend  = FALSE, # to hide the `a` from the legend
...
ggplot(data = fit_and_meta_df, aes(x = V1, y = V3, label = AbbreviatedName, shape = TopLevel, color = Type)) +
  geom_point(size=2) + 
  geom_text_repel(
    size=3,
    show.legend  = FALSE, # to hide the `a` from the legend
